...
- 단 🔹 Elastic Net 계수 조정를 잘하면 성능 좋아짐.
lambda_l1 = 1e-3, lambda_l2 = 1e-3 로 수치를 잘 조작하면 좋아짐. 
...
- (1) λ가 과도하게 큼: _lambda = 0.5 는 MSE 스케일 대비 매우 큰 값

- L1: 비미분점 + 강한 패널티. 작은 모델에서 표현력 급격히 훼손

- L2: 과도한 shrinkage(수축) 발생. 👉 정칙화 개념의 실패가 아니라, 하이퍼파라미터 튜닝 실패

- (2) 데이터 자체가 이미 과적합 상태가 아님 -> 정칙화 없이도 train / val / test loss 모두 안정
- test loss ≈ 0.096: 충분히 작아서 고칠 과적합이 존재하지 않음. 👉 문제가 없는데 제약을 건 상황

- (3) 더욱이 이 문제에서는 feature를 줄이는 것은 부적합할 수 있음.

- * 입력 feature: [x², x] (2개뿐)
- * L1의 본질: sparsity / feature selection

- 두 feature 모두 의미 있음. 하나라도 약화 → 표현력 손실 → 성능 저하 👉 L1의 장점이 단점으로 작동한 전형적인 사례

- (4) Gradient Clipping & Dropout이 망가진 이유 (보완 설명)

- 🔹 Gradient Clipping 원래 목적: gradient 폭발 방지지만 여기서는 폭발 문제가 없음
- max_norm=0.1 → 학습 신호 자체 제거 (그래디언트를 너무 작게 제한함) 👉 underfitting → 학습 붕괴
- 그래디언트 클리핑이란 gradient의 크기(norm)가 기준을 넘으면, 그 크기를 기준값까지 “줄여서(clipping해서)” 쓰라는 뜻 
결과적으로 잘 작동하던 그래디언트를 과도하게 제약해서 학습이 붕괴됨.

- 🔹 Dropout
- 회귀 문제 + 데이터 적음 + 출력 1차원
-Dropout(p=0.5): 출력 분산을 인위적으로 흔듦. MSE 기반 회귀에서 치명적 👉 노이즈 주입 → 예측 자체 붕괴

### 5️⃣ 회고

- 정칙화는 항상 성능을 높여주는 마법이 아니라,
과적합이 존재하고, 적절한 강도로 적용될 때만 효과가 있음.
- 이번 실험에서는 그 조건이 성립하지 않았음.

- L1 / L2 / Dropout / Gradient Clipping → 과도하거나 부적절

- Weight Decay, Momentum → 약한 제어로 소폭 개선

- ** 가장 중요한 것은 문제·데이터·모델에 맞는 기법 선택하는 것으로 이 실험의 진짜 성과는 정칙화를 언제 쓰지 말아야 하는지를 확인했음.
